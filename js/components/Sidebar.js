        ...
        <aside class="hidden md:flex w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex-col z-20 shadow-lg shrink-0">
            ...
            <div class="p-4 border-t border-slate-100 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-800/50">
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <button @click="selectLocalFileDirectory" class="col-span-2 flex items-center justify-center gap-1 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg text-xs font-bold transition-colors" :class="localFileStoragePath ? 'text-amber-600 dark:text-amber-400' : 'text-amber-500 dark:text-amber-500'" :title="localFileStoragePath || '点击设置本地存储路径'"><i class="ph ph-folder-simple"></i> {{ localFileStoragePath ? '📁 ' + localFileStoragePath : '设置本地路径' }}</button>
